...
class RBM(object):
  def __init__(self, name, isize, osize, learning_rate=0.01, CDk=1):
    ...
    self._weight = tf.Session().run(
        tf.random_normal([isize, osize], dtype=tf.float64))
    self._v_bias = tf.Session().run(
        tf.random_normal([isize], dtype=tf.float64))
    self._h_bias = tf.Session().run(
        tf.random_normal([osize], dtype=tf.float64))
  ...
  def _0_1_sample_given_p(self, p):
    return tf.nn.relu(tf.sign(p - tf.random_uniform(tf.shape(p),dtype=np.float64)))
  ...
  def rbm_train(self, data_x, batch_size=128, n_epoches=1, verbose=True):
    x_in = tf.placeholder("float64", shape=[None, self._input_size])
    ...
    with tf.Session() as sess:
      ...
      for epoch in range(n_epoches):
        ...
        for i_batche in range(n_batches):
          ...
          recon_lost = sess.run(
              x_loss, feed_dict={x_in: batch_x})  # 误差应该在train之前计算
